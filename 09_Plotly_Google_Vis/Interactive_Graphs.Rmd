...
#  all_sum1<-mean(count$Total)
  bar1<-ggplot(data=count, aes(count$cohort, fill=count$cohort)) + geom_bar(aes(weight=count$Total)) +  
    ...
    geom_text(aes(x=count$cohort, y=count$Total, label=count$Total), vjust=-1) + ggtitle("Figure #1 - Total Number of Fiancés as Operators by Cohort") 
  plot(bar1)
```
...
#Create Google Visualizations  
  ...
  '
  m_reg<-gvisMotionChart(count_l[1:132,c(1:4,7)], idvar="cohort", timevar = "Month", xvar = "Month")
