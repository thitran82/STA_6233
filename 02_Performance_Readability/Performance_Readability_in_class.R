...
for(i in 1:10){
  print(c("number", i), quote=F)
}

for(i in 1:10){
  print(paste0("number+", i, " A"))
}
...
names_list <- c("Bob", "Bill", "Sue", "Antetokounmpo")
letters_list <- c("A","B","C")
numbers_list <- 6:10

for (i in seq_along(names_list)){
  for (j in seq_along(letters_list)){
    for (k in seq_along(numbers_list))
      print(paste(names_list[i],paste0(letters_list[j],numbers_list[k]),sep = " "))
  }
}
...
while(i<10000){
  ...
  print(paste("iteration=",iteration,i,"Still Below 10,000",sep=" "))
  i=i+(i*i)
}
...
dat$Risks_fun<-first_fun(dat,dat$Risks)
head(dat[,c(3:14:15)])
...
dat$WorkValued_t<-sec_fun(dat$Risks)
head(dat[,c(3:14:16)])
